...
function AddMenuPed(menu)
	...
	male = NativeUI.CreateItem('Create New Male Character',"WARNING: Deletes existing look")
	menu:AddItem(male)
	female = NativeUI.CreateItem('Create New Female Character',"WARNING: Deletes existing look")
	menu:AddItem(female)
	existing = NativeUI.CreateItem('Modify Current Appearance', "Modify Current Appearance")
	menu:AddItem(existing)
	savechar = NativeUI.CreateItem('Save Character', "Save Appearance")
	menu:AddItem(savechar)
	...
	freemodePedMenu.OnMenuClosed = function(Menu)
		...
		ESX.ShowNotification("Skin Saved!")
        --print("Skin Saved")
        DeleteSkinCam()
		menu:Visible(true)
	end
...
mainMenu = NativeUI.CreateMenu("Character Creation Menu", "~b~Create your character", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
...
local submenuInheritance = _menuPool:AddSubMenu(freemodePedMenu, "Heritage", "~b~Choose your Heritage", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
local submenuFaceShape = _menuPool:AddSubMenu(freemodePedMenu, "Face Shape", "~b~Character Face Shape Options", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)
      ...
      submenuAccessories = _menuPool:AddSubMenu(freemodePedMenu, "Accessories", "~b~Head Accessory Options", Config.MenuX, Config.MenuY,nil,nil,nil,Config.MenuColor.r,Config.MenuColor.g,Config.MenuColor.b,Config.MenuColor.a)


	local cMum,cDad = 1
	local SkinMixData,ShapeMixData = 0.5
function AddMenuInheritance(submenu)
    zoomOffset = Components[1].zoomOffset
    camOffset = Components[1].camOffset
...
function AddMenuFaceShape(submenu)
	...
	local ListItem = {
		...
		local GridPanel = NativeUI.CreateGridPanel("Crooked", "Long", "Short", "Curved")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateGridPanel("Bridge In", "Tip Up", "Tip Down", "Bridge Out")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateGridPanel("In", "Up", "Down", "Out")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateHorizontalGridPanel("Puffed", "Gaunt")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateHorizontalGridPanel("Wide", "Squint")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateHorizontalGridPanel("Fat", "Thin")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
		...
		local GridPanel = NativeUI.CreateGridPanel("Rounded", "Pointed", "Square", "Bum")
		submenu.SubMenu:AddItem(GridItem)
		GridItem:AddPanel(GridPanel)
		GridItem.Base.ActivatedPanel = function(_, item, panel, panelvalue)
...
function AddMenuAppearance(submenu)
	...
	local maxDrawables = GetNumberOfPedDrawableVariations(GetPlayerPed(-1), 2); -- Hair - 2
        maxDrawables = maxDrawables - 1
        amount = {}
        for i = 1, maxDrawables do
            amount[i] = "Style "..i.."/"..maxDrawables
		end
	...
	local Select = NativeUI.CreateListItem("Haircut", amount , 1, Description )
	local BaseColor = NativeUI.CreateColourPanel("Colour", ColoursPanel.HairCut)
	local HighColor = NativeUI.CreateColourPanel("Highlight Colour", ColoursPanel.HairCut)
	Select:AddPanel(BaseColor)
	Select:AddPanel(HighColor)
    ...
    Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
		...
		local highlightcolor = (ActiveItem.Panels and ActiveItem.Panels[2] or 1)
		SetPedHairColor(GetPlayerPed(-1),color-1,highlightcolor-1)					-- Hair Color
		Character['hair_1'] = Index-1
		Character['hair_color_1'] = color-1
		Character['hair_color_2'] = highlightcolor-1
	end
...
function AddMenuAccessories(submenu)

    AccessoryCategories = {
        {PropID = 0, Label = "Hats", Name = "helmet", Zoom = 0.6, Cam = 0.65},
        {PropID = 1, Label = "Glasses", Name = "glasses", Zoom = 0.6, Cam = 0.65 },
        {PropID = 2, Label = "Ears", Name = "ears", Zoom = 0.6, Cam = 0.65},
        {PropID = 6, Label = "Watches", Name = "watches", Zoom = 0.75, Cam = 0.15 },
        {PropID = 7, Label = "Bracelets", Name = "bracelets", Zoom = 0.75, Cam = 0.15}

    }
    local maxvals = {}
    TriggerEvent("skinchanger:getData", function(components,_maxvals)
        maxvals = _maxvals end)

    local amount = {}
    for i=1, #AccessoryCategories do
        local maxDrawables = maxvals[AccessoryCategories[i].Name .. "_1"]
        maxDrawables = maxDrawables - 1
        --print("MaxDrawables = "..maxDrawables)
        amount = {}
        for i = 1, maxDrawables do
            amount[i] = "item "..i.."/"..maxDrawables
        end
        local Select = NativeUI.CreateListItem(AccessoryCategories[i].Label, amount , 1, "Choose your "..AccessoryCategories[i].Label )
        ...
        Select.OnListChanged = function(ParentMenu, SelectedItem, Index)
            ...
            local color = (ActiveItem.Panels and ActiveItem.Panels[1] or 1)
			SetPedPropIndex(GetPlayerPed(-1),AccessoryCategories[i].PropID,Index-1,color-1,true)
            
			
			
            Character[AccessoryCategories[i].Name .. "_1"] = Index-1
			Character[AccessoryCategories[i].Name .. "_2"] = color-1
			zoomOffset = AccessoryCategories[i].Zoom
            camOffset = AccessoryCategories[i].Cam
...
AddMenuAccessories(submenuAccessories)
