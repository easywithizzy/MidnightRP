...
Citizen.CreateThread( function()
	...
	while true do
		Wait(0)
		if IsControlJustPressed(1, 183) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Initializing radar...~w~321...~y~Loaded! ")
				radar.info2 = string.format("~y~Initializing radar...~w~321...~y~Loaded! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if IsControlJustPressed(1, 182) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
		
			if radar.freeze then radar.freeze = false else radar.freeze = true 
			notify("Radar ~r~Locked")
			end
		end
		if radar.shown  then
			...
			DrawAdvancedText(0.591, 0.903, 0.005, 0.0028, 0.4, "Front Radar", 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.591, 0.953, 0.005, 0.0028, 0.4, "Rear Radar", 255, 255, 255, 255, 6, 0)
